 ...
 *
 * GET /{id} — Lab 1: missing @PreAuthorize (SonarQube finds it); fix adds PII
 * masking
 * POST / — @Valid on request + @PreAuthorize('MANAGER') — secure example
 * PUT /{id}/status — state machine enforced at service layer (A04 Insecure
 * Design)
 * GET /search — CAPSTONE LAB: SQL injection vulnerability (string concatenation
 * JPQL)
